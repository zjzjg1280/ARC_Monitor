...
class ArcEventWidget:
    def __init__(self, root):
        ...
        self.root.geometry("500x650+100+100")

        # --- 字体 ---
        self.font_size = 10

        self.font_title = tkfont.Font(family="Consolas", size=self.font_size + 2, weight="bold")
        self.font_content = tkfont.Font(family="Consolas", size=self.font_size)
        self.font_card_header = tkfont.Font(family="Consolas", size=self.font_size + 1, weight="bold")
        ...
        # 标题文本修改为英文
        self.title_label = tk.Label(self.header_frame, text="Arc Raiders Tracker",
    ...
    def fetch_data(self):
        try:
            ...
            for item in events_list:
                ...
                # --- 取消翻译部分 (直接使用原始英文名称) ---
                event_name = raw_name  # cn_name = TRANSLATIONS.get(raw_name, raw_name)
                location_name = raw_loc  # cn_loc = TRANSLATIONS.get(raw_loc, raw_loc)
                # ---------------------------------------------
                ...
                for k, v in item.items():
                    if isinstance(v, list) and len(v) > 0 and isinstance(v[0], dict) and "start" in v[0]:
                        ...
                        break
                if schedule:
                    if location_name not in grouped: grouped[location_name] = []
                    grouped[location_name].append({"name": event_name, "schedule": schedule})
                ...
                for event in items:
                    ...
                    for start, end in time_slots:
                        if start <= now <= end:
                            ...
                            hh, mm = divmod(mm, 60)
                            t_str = f"{hh}:{mm:02d}" if hh > 0 else f"{mm}m {ss}s"
                    ...
                    if not found_active:
                        ...
                        if future:
                            ...
                            hh, mm = divmod(mm, 60)
                            w_str = f"{hh}:{mm:02d}" if hh > 0 else f"{mm}m"
